<template>
    <div class="container mx-auto px-4 py-12">
        ...
        <div v-else-if="pageContent" class="max-w-4xl mx-auto">
            ...
            <!-- Last Updated Info -->
            <div
                v-if="pageContent.updatedAt || pageContent.createdAt"
                class="mt-12 pt-6 border-t border-gray-200 text-sm text-gray-500"
            >
                Last updated:
                {{ formatDate(pageContent.updatedAt || pageContent.createdAt) }}
            </div>
        </div>
...
const fetchPageContent = async () => {
    ...
    try {
        ...
        // Fetch the page content using the dynamic-content endpoint
        ...
        if (response.data) {
            ...
            document.title = `${response.data.title} | Your Site Name`;
        } else {
        ...
        document.title = "Error | Your Site Name";
...
// Watch for route changes to reload content when navigating between pages
watch(
    () => route.params.slug,
    (newSlug) => {
        if (newSlug) {
